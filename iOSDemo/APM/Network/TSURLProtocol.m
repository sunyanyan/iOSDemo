...
@interface TSURLProtocol() <NSURLConnectionDelegate, NSURLConnectionDataDelegate>
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    ...
    // 如果是已经拦截过，放行
    if ([NSURLProtocol propertyForKey:kTSURLRequestKey inRequest:request]) {
        return NO;
    }
    return YES;
}
...
+ (NSURLRequest *)canonicalRequestForRequest:(NSURLRequest *)request {
    ...
    // HTTPDNS 域名转 iP
    [NSURLProtocol setProperty:@(YES) forKey:kTSURLRequestKey inRequest:mutableRequest];
...
- (void)stopLoading {
    ...
    for (id<TSURLProtocolDelegate> delegate in [[self class]hashTable]) {
